...
def generate_mandelbrot_set(points, num_processes):
    ...
    chunks = [points[i*chunk_size:(i+1)*chunk_size] for i in range(num_processes - 1)]
    chunks.append(points[(num_processes-1)*chunk_size:])

    pool = multiprocessing.Pool(num_processes)
    results_async = [pool.apply_async(compute_chunk, (chunk,)) for chunk in chunks]
    escape_times = []
    for r in results_async:
        escape_times.extend(r.get())
